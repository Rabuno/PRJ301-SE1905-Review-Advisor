...
public class SqlProductDAO implements IProductRepository {
    ...
    private Product mapRow(ResultSet rs) throws Exception {
        ...
        try {
            ...
            if (status != null) {
                p.setStatus(domain.enums.ProductStatus.valueOf(status));
            }
        } catch (Exception e) {
            // status column might not be present in all queries
        }
        return p;
    }
    ...
    public List<Product> findByCategory(String category) {
        List<Product> list = new ArrayList<>();
        String sql = "SELECT product_id, name, category, description, price, merchant_id, image_url, status "
                + "FROM Products WHERE category = ? AND status = 'ACTIVE' ORDER BY created_at DESC";
        try (Connection conn = DBConnection.getConnection();
                PreparedStatement ps = conn.prepareStatement(sql)) {
