...
<div class="container mt-4 mb-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted small mb-0">Manage your properties and monitor customer feedback via Data Visualizations.</p>
        </div>
        ...
        <div class="col-md-3">
            <div class="card shadow-sm border-0 border-start border-4 border-primary h-100">
                <div class="card-body">
                    ...
                    <div class="h3 mb-0 text-dark fw-bold">${not empty requestScope.STATS.totalProperties ? requestScope.STATS.totalProperties : '0'}</div>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card shadow-sm border-0 border-start border-4 border-warning h-100">
                <div class="card-body">
                    ...
                    <div class="h3 mb-0 text-dark fw-bold">${not empty requestScope.STATS.avgRating ? requestScope.STATS.avgRating : '0.0'} <span class="text-warning small">★</span></div>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card shadow-sm border-0 border-start border-4 border-success h-100">
                <div class="card-body">
                    ...
                    <div class="h3 mb-0 text-dark fw-bold">${not empty requestScope.STATS.publishedCount ? requestScope.STATS.publishedCount : '0'}</div>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card shadow-sm border-0 border-start border-4 border-danger h-100">
                <div class="card-body">
                    ...
                    <div class="h3 mb-0 text-danger fw-bold">${not empty requestScope.STATS.flaggedCount ? requestScope.STATS.flaggedCount : '0'}</div>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <c:choose>
                            <c:when test="${not empty requestScope.RECENT_FEEDBACK}">
                                <c:forEach var="feedback" items="${requestScope.RECENT_FEEDBACK}">
                                    <li class="list-group-item p-3 ${feedback.status == 'FLAGGED' ? 'bg-light border-start border-3 border-danger' : ''}">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <strong class="small text-dark">Product ID: ${feedback.productId}</strong>
                                            <c:choose>
                                                <c:when test="${feedback.status == 'FLAGGED'}">
                                                    <span class="badge bg-danger small">Flagged / Pending</span>
                                                </c:when>
                                                <c:otherwise>
                                                    <span class="badge bg-success small">${feedback.rating} ★</span>
                                                </c:otherwise>
                                            </c:choose>
                                        </div>
                                        <div class="small text-muted mt-1 ${feedback.status == 'FLAGGED' ? 'fst-italic' : ''}">"${feedback.content}"</div>
                                    </li>
                                </c:forEach>
                            </c:when>
                            <c:otherwise>
                                <li class="list-group-item p-4 text-center text-muted small">No recent feedback available.</li>
                            </c:otherwise>
                        </c:choose>
                    </ul>
                </div>
