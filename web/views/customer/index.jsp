                        ...
                        <c:forEach var="product" items="${requestScope.PRODUCT_LIST}">
                            <div class="col-md-4 mb-4">
                                <div class="card h-100 shadow-sm card-product border-0 rounded-3 overflow-hidden">
                                    <img src="${not empty product.imageUrl ? product.imageUrl : 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600&h=400&fit=crop'}"
                                    ...
                                    <div class="card-body d-flex flex-column">
                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                            ...
                                            <span class="badge ${product.status == 'ACTIVE' ? 'bg-success-subtle text-success border-success' : 'bg-secondary-subtle text-secondary border-secondary'} border rounded-pill">
                                                ${not empty product.status ? product.status : 'ACTIVE'}
                                            </span>
                                        </div>

                                        <p class="text-muted small mb-3">
                                            <i class="bi bi-card-text text-danger"></i> Description: 
                                            <span title="${product.description}">
                                                <strong>${product.description.length() > 50 ? product.description.substring(0, 50).concat('...') : product.description}</strong>
                                            </span>
                                        </p>
